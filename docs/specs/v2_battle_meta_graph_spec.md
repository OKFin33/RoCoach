...
Status: implemented — H-Graph pipeline live, S-Graph deferred to V2.1
Author: Clé (spec), Coco (implementation)
Date: 2026-05-03 (spec), 2026-05-15 (implementation)
...
## 附录 C：实现状态 (2026-05-15)

### 已实现

**数据目录**：`artifacts/v2_meta_graph/`
- `species_sets/` — 物种配置卡（YAML），含模板 `_template.yaml`
- `graph_registry.yaml` — 手动维护的注册表
- `edge_index.yaml` — 由 `v2_generate_edge_index.py` 自动生成
- `speed_index.yaml` — 由 `v2_generate_speed_index.py` 自动生成

**工具链**（均在 `tools/`）：
| 工具 | 功能 |
|---|---|
| `v2_meta_graph_contracts.py` | 共享枚举、卡片 I/O、ID 生成 |
| `v2_validate_graph.py` | 单卡 + 跨卡 + 索引一致性校验 |
| `v2_generate_edge_index.py` | 扫描 related_to → 边索引 |
| `v2_generate_speed_index.py` | 扫描 speed_tier → 速度索引 |
| `v2_meta_graph_resolver.py` | 模糊匹配（社区名→Battle Dex）+ 默认配置解析 + 角色冲突检测 |

**运行时检索**：`advisor/meta_graph.py`
- `MetaGraphRetriever` 类：按物种/标签/角色查询、关系查询、速度对比、prompt context block 组装
- `get_meta_graph_retriever()` — 模块级单例

**首批数据**：光合武队 6 张卡（`review_status: unreviewed`）
- 针叶巡林（首发/印记）、食尘短绒（地刺/联防）、芋香巨角蛛（天气/控场）、声波缇塔（中速/地刺）、布克棱岩（抗压/前排）、棋绮后（主C/收割）
- 11 条关系、6 个速度条目、14 条速度关系

### 已定案的决策（原 spec Section 11）

| 决策点 | 结论 |
|---|---|
| D2 Review bar | **接受。** 单维护者，PM 看过合理 → `reviewed` |
| D3 `observed` 标准 | **接受。** 一个天梯高分玩家在视频中明确声称 → `observed` |
| D8 最小信息门槛 | **强制。** `description` + 至少一个 `evidence_ref` |
| 三个 OCR 名称校正 | 已处理。食尘短蛛→食尘短绒、宇雾巨角蛛→芋香巨角蛛、声波提塔→声波缇塔 |

### 仍待 PM 决策

| 决策点 | 状态 |
|---|---|
| D1 Meta snapshot 粒度 | 当前用 `2026-s1`（赛季级），待确认 |
| D4 Graph 文件是否进 public repo | 待决策 |
| D5 V2 启动时机 | **已前置。** species_set 图谱改为 V1 发版前置条件 |
| D6/D7 Shadow Graph 阈值/升级标准 | V2.1 的事，不现在定 |

### spec 与实现的已知差异

- 模糊匹配阈值：当前 `overlap >= 0.6`，短名（2-3 字）会产生误匹配（如"不存在精灵"→"画精灵"）。后续考虑加最短名称长度门槛。
- 速度值计算采用近似公式 `floor((2*base + 36) * nature_mult)`。如 Roco 实际公式不同，需修正 `stat_profile.speed` 计算方式。
- `S-Graph` 和 `agent_synthesis` 为占位，枚举已定义但检索模块有硬门控（`shadow_graph_enabled=False` 时不检索）。
