...
def _species_answer_summary(
    ...
    initial_name = getattr(profile, "initial_species_name", None)
    if initial_name and initial_name != profile.display_name:
        display_name = f"{profile.display_name}（{initial_name}进化线最终形态）"
    summary = (
        f"先按对战默认形态看：{display_name} 是{profile.primary_type}"
...
def _tool_evidence_ids(tool_name: str, evidence_by_label: dict[str, str]) -> list[str]:
    ...
    if tool_name == "analyze_team_structure":
        return [evidence_id for label, evidence_id in evidence_by_label.items() if label == "engine.team_structure"]
